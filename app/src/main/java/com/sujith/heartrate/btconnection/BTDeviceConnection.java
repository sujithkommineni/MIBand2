...
public class BTDeviceConnection extends BluetoothGattCallback {
    ...
    public void getBatteryStatus(final ResponseListener listener) {
        Timber.v("getBatteryStatus()..");
        if (!mConnectionSuccessful) {
            Timber.i("Connection to BT device is not ready yet.. ignoring");
            return;
        }
        final byte[] battery = PacketUtils.batteryStatusPacket();
        mWorkerHandler.post(new Runnable() {
            @Override
            public void run() {
                final byte[] data = writeAndWaitForResponse(battery);
                mUiHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        listener.onResponse(PacketParserUtils.parseBatteryPacket(data));
                    }
                });
            }
        });
    ...
    public void getBTDeviceSWVersion(final ResponseListener listener) {
        Timber.v("getBTDeviceSWVersion()..");
        if (!mConnectionSuccessful) {
            Timber.i("Connection to BT device is not ready yet.. ignoring");
            return;
        }
        final byte[] version = PacketUtils.versionQueryPacket();
        mWorkerHandler.post(new Runnable() {
            @Override
            public void run() {
                final byte[] data = writeAndWaitForResponse(version);
                mUiHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        listener.onResponse(PacketParserUtils.parseAppVersion(data));
                    }
                });
            }
        });
    ...
    public void setSessionData(final ResponseListener listener) {
        Timber.v("setSessionData()..");
        if (!mConnectionSuccessful) {
            Timber.i("Connection to BT device is not ready yet.. ignoring");
            return;
        }
        final byte[] sessionData = PacketUtils.setSessionData(Calendar.getInstance().getTime(), 0, 1);
        mWorkerHandler.post(new Runnable() {
            @Override
            public void run() {
                final byte[] data = writeAndWaitForNoResponse(sessionData);
                mUiHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        listener.onResponse(PacketParserUtils.parseSetSessionDataResponse(data));
                    }
                });
            }
        });
    ...
    public void startSession(final ResponseListener listener, boolean start) {
        Timber.d("startSession().. " + (start ? "START" : "STOP"));
        if (!mConnectionSuccessful) {
            Timber.i("Connection to BT device is not ready yet.. ignoring");
            return;
        }
        final byte[] startSession = PacketUtils.startTrainingSession(start);
        mWorkerHandler.post(new Runnable() {
            @Override
            public void run() {
                final byte[] data = writeAndWaitForNoResponse(startSession);
                mUiHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        listener.onResponse(PacketParserUtils.parseStartSessionResponse(data));
                    }
                });
            }
        });
    ...
    public void getLogData(final ResponseListener listener) {
        Timber.d("getLogData()..");
        if (!mConnectionSuccessful) {
            Timber.i("Connection to BT device is not ready yet.. ignoring");
            return;
        }
        final byte[] getLogData = PacketUtils.getLogData(0);
        mWorkerHandler.post(new Runnable() {
            @Override
            public void run() {
                writeAndWaitForMultipleResponses(getLogData);
                final ArrayList<Byte> response = new ArrayList<>();
                response.addAll(mResponseList);
                mUiHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        //listener.onResponse(PacketParserUtils.parseGetLogDataResponse(data));
                        listener.onResponse(new GetLogDataResponse(true, response));
                    }
                });
            }
        });
